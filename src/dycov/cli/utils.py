...
def get_dynawo_launcher_name(parser: argparse.ArgumentParser, args: argparse.Namespace) -> str:
    ...
    """
    dycov_logging.get_logger("CliUtils").debug("Getting Dynawo launcher name.")
    ...
    if not dynawo_launcher_name:
        ...
        if shutil.which(dynawo_launcher_name) is None and args.command != "anonymize":
            dycov_logging.get_logger("CliUtils").critical(
    ...
    dycov_logging.get_logger("CliUtils").info(
        f"Dynawo launcher resolved to: {dynawo_launcher_name}"
    )
...
def check_dynawo_launcher_availability(dynawo_launcher_name: str) -> None:
    ...
    """
    dycov_logging.get_logger("CliUtils").info(
        f"Checking Dynawo launcher availability: {dynawo_launcher_name}"
    )
    try:
        # Calls the launcher check function from the sanity_checks module.
        system_checks.check_launchers(dynawo_launcher_name)
        dycov_logging.get_logger("CliUtils").debug(
            f"Dynawo launcher '{dynawo_launcher_name}' is available."
        )
    except OSError as e:
        # If an OSError occurs (due to missing launchers), logs it and exits.
        dycov_logging.get_logger("CliUtils").critical(f"Dynawo launcher check failed: {e}")
        sys.exit(1)
