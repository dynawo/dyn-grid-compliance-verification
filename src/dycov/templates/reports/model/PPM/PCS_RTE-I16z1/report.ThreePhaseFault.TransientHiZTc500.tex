    ...
    \subsection{Transient fault: High-impedance fault (500 ms, $V=0.7 U_{dim}$)}

    Checks for compliant behavior of the generating unit under a grid scenario where there
    is a transient three-phase impedant fault, causing a voltage dip of 0.7 $U_{dim}$.
    ...
    \noindent Dynamic behaviour: {{ stabilizedPCSI16z1ThreePhaseFaultTransientHiZTc500 }}
    ...
    \begin{center}
        ...
        \begin{tabular}{cllc}
            ...
            \midrule
            \BLOCK{for row in ssemPCSI16z1ThreePhaseFaultTransientHiZTc500}
            {{row[0]}} & {{row[1]}} & {{row[2]}}  & {{row[3]}} \\
            \BLOCK{endfor}
            \bottomrule
        \end{tabular}
