    ...
    \renewcommand{\OCname}{TransientHiZTc800}


    \subsection{Transient fault: High-impedance fault (800 ms, $V=0.5 U_{dim}$)}

    Checks for compliant behavior of the generating unit under a grid scenario where there
    is a transient three-phase impedant fault, causing a voltage dip of 0.5 $U_{dim}$.
    ...
    \begin{center}
        \begin{tabular}{lc}
           ...
           \textbf{Parameter} & \textbf{Value (default)} \\
            \midrule
            \BLOCK{for row in solverPCSI16z1ThreePhaseFaultTransientHiZTc800}
            {{row[0]}}         & {{row[1]}}                         \\
            \BLOCK{endfor}
            \bottomrule
        \end{tabular}
    ...
    \GridCurvesItZone
    \\[2\baselineskip]
    Go to  {{ linkPCSI16z1ThreePhaseFaultTransientHiZTc800 }}

    \subsubsection{Analysis of results}

    \noindent Dynamic behaviour: {{ stabilizedPCSI16z1ThreePhaseFaultTransientHiZTc800 }}
    ...
    \begin{center}
        ...
        \begin{tabular}{@{}lccccccccc@{}}
            ...
            \midrule
            \BLOCK{for row in rmPCSI16z1ThreePhaseFaultTransientHiZTc800}
    ...
    \begin{center}
        ...
        \begin{tabular}{@{}lccccccccc@{}}
            ...
            & {MXE}      & {ME}       & {MAE}      & {MXE}      & {ME}       & {MAE}      & {MXE}      & {ME}       & {MAE}      \\
            \midrule
            \BLOCK{for row in thmPCSI16z1ThreePhaseFaultTransientHiZTc800}
            {{row[0]}} & {{row[1]}} & {{row[2]}} & {{row[3]}} & {{row[4]}} & {{row[5]}} & {{row[6]}} & {{row[7]}} & {{row[8]}} & {{row[9]}} \\
            \BLOCK{endfor}
            \bottomrule
        \end{tabular}
    ...
    \begin{center}
        ...
        \begin{tabular}{@{}lcccccccccc@{}}
            ...
            \midrule
            \BLOCK{for row in emPCSI16z1ThreePhaseFaultTransientHiZTc800}
    ...
    \begin{center}
        ...
        \begin{tabular}{cllc}
            ...
            \midrule
            \BLOCK{for row in aprPCSI16z1ThreePhaseFaultTransientHiZTc800}
    ...
    \begin{center}
        ...
        \begin{tabular}{cllc}
            ...
            \midrule
            \BLOCK{for row in ssemPCSI16z1ThreePhaseFaultTransientHiZTc800}
            {{row[0]}} & {{row[1]}} & {{row[2]}}  & {{row[3]}} \\
            \BLOCK{endfor}
            \bottomrule
        \end{tabular}
