...
class AmplitudeStep(GFMCalculator):
    ...
    def _get_delta_iq(
        ...
        Returns:
            tuple[np.ndarray, np.ndarray, np.ndarray]: A tuple with the baseline delta_iq,
    ...
    def _get_envelopes(
        ...
        Args:
            delta_iq_base (np.ndarray): The baseline reactive current change array.
            delta_iq_min (np.ndarray): The minimum boundary array.
            delta_iq_max (np.ndarray): The maximum boundary array.
            Xeff (float): The effective reactance.

        Returns:
            tuple[np.ndarray, np.ndarray, np.ndarray]: A tuple containing the expected
    ...
    def _get_delta_iq_base(self, Xeff: float, time_array: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: The mathematical baseline reactive current array.
        """
    ...
    def _calculate_delta_iq_base(self, Xeff: float, time_array: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: The baseline reactive current array.
        """
    ...
    def _get_delta_iq_min(self, Xeff: float, time_array: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: The array representing the minimum expected current trajectory.
        """
    ...
    def _get_delta_iq_max(self, Xeff: float, time_array: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: The array representing the maximum expected current trajectory.
        """
        tunnel = self._get_tunnel(Xeff)
        voltage_step_pu = self._voltage_step / 100.0
        max_delta_iq = np.abs(voltage_step_pu / (Xeff + self._Xgrid))
    ...
    def _get_tunnel(self, Xeff: float) -> float:
        ...
        Returns:
            float: The permitted tunnel margin value.
        """
        voltage_step = self._voltage_step / 100.0
