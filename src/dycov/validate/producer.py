...
class ModelProducer(Producer):
    ...
    def __init__(
        ...
        super().__init__(
            producer_model_path if producer_model_path else producer_curves_path, None
        )
        self._producer_curves_path = producer_curves_path
        self._reference_curves_path = reference_curves_path
        self._zone = 0

        self._is_dynawo_model = producer_model_path is not None
        self._is_user_curves = producer_curves_path is not None
    ...
    def __get_file_by_pattern(self, pattern) -> Path:
        if self.get_producer_path() is not None:
            producer_path = self.get_producer_path()
        else:
    ...
    def set_zone(self, zone: int) -> None:
        ...
        if self.is_dynawo_model():
            ...
            sanity_checks.check_curves_files(
                self.get_producer_path(), self._reference_curves_path, self.get_sim_type_str()
            )
            self.__init_model()
