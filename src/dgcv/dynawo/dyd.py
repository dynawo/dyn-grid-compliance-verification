    ...
    def complete_file(self, working_oc_dir: Path, event_params: dict) -> None:
        ...
        if event_params["connect_to"]:
            connect_event_to = dynawo_translator.get_dynawo_variable(
                self._producer_curves.get_producer().generators[0].lib, event_params["connect_to"]
            )
            variables_dict["generator_id"] = self._producer_curves.get_producer().generators[0].id
